...
int readImageDataAssembly(FileInfo *fileValues, long nImageBytes, FILE *inputFile, char* filename, int inputRowStart, int inputRowEnd, int inputColumnStart, int inputColumnEnd){
  if (access(filename, R_OK) == -1){
    printErrorStringAndDescription(EXIT_BAD_INPUT_FILE, filename);
    return EXIT_BAD_INPUT_FILE;
  }
  ...
  // For each value in file, read it, check it and write its pointer into imageData if valid
  int amountOfValues=0;
  int currentColumn=inputColumnStart, currentRow= inputRowStart;
  for (unsigned char *nextGrayValue = fileValues->imageData; nextGrayValue < fileValues->imageData + nImageBytes; nextGrayValue++){
    int grayValue = -1;
    // Retrieve the value for grayValue from file
    int scanCount = fscanf(inputFile, " %u", &grayValue);
    ...
    *nextGrayValue = (unsigned char) grayValue;
    //map[currentRow][currentColumn] = grayValue;
    amountOfValues++;
    if (currentColumn==(fileValues->width-1)){
      currentColumn=0;
      currentRow+=1;
    }
    else{
...
// int readImageDataBinaryAssembly(FileInfo *fileValues, long nImageBytes, FILE *inputFile, char* filename, int inputRowStart, int inputRowEnd, int inputColumnStart, int inputColumnEnd, unsigned char * map){
//   if (access(filename, R_OK) == -1){
//     printErrorStringAndDescription(EXIT_BAD_INPUT_FILE, filename);
//     return EXIT_BAD_INPUT_FILE;
//   }
